...
public class AccountController {
    ...
    @PutMapping("/password")
    public CommonResult<?> changePassword(@RequestBody PasswordDTO passwordDTO) {
    ...
    @GetMapping()
    public CommonResult<AccountVO> queryInfo() {
        Long userId = UserContext.get();
        log.info("用户查询个人信息：用户id = {}", userId);
        return CommonResult.success(accountService.queryById(userId));
    }

    @Operation(summary = "修改个人信息")
    @PutMapping("/edit")
    public CommonResult<?> modifyInfo(@RequestBody AccountDTO accountDTO) {
        Long userId = UserContext.get();
    ...
    public CommonResult<PageVO<ConferenceVO>> queryFollowingConferencesByPage(ConferencePageQueryDTO query) {
        Long userId = UserContext.get();
        log.info("用户分页查询已关注的会议：查询条件 = {}，用户id = {}", query, userId);
        return CommonResult.success(conferenceService.queryFollowingConferencesByPageAndUserId(userId, query));
    }
}
